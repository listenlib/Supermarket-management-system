...
function deleteProvider(obj){
	$.ajax({
		type:"GET",
		url:path+"/provider/delProvider.html",
...
$(function(){
	$(".viewProvider").on("click",function(){
		...
		var obj = $(this);
		window.location.href=path+"/provider/provider.do?method=view&proid="+ obj.attr("proid");
	});
	
	$(".modifyProvider").on("click",function(){
		var obj = $(this);
		window.location.href=path+"/provider/provider.do?method=modify&proid="+ obj.attr("proid");
	});
